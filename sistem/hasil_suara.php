...
if(isset($_POST['simpan'])) {
  ...
  $nm_calonlon_wakil= mysqli_real_escape_string($koneksi, $_POST['nm_calon_wakil']);
  ...
  if($_POST['simpan']){
    ...
    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
      if($ukuran < 5044070){          
        ...
        $query = mysqli_query($koneksi, "INSERT INTO data_calon VALUES(NULL, '$gambar1')");
        ...
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
          if($ukuran < 5044070){          
            ...
            $query = mysqli_query($koneksi, "INSERT INTO data_calon VALUES(NULL, '$gambar2')");
          }
        }
      }
    }
  ...
  mysqli_query($koneksi,"INSERT INTO data_calon(id, nim_ketua, nm_calon_ketua, gambar1, nim_wakil, nm_calon_wakil, gambar2, no_urut)
    VALUES ('','$nim_ketua','$nm_calon_ketua','$gambar1','$nim_wakil','$nm_calon_wakil','$gambar2','$no_urut')");

  echo "<script>window.alert('Berhasil')
  window.location='input_data_calon.php'</script>";
      ...
      <div class="menu">
        <ul class="nav" id="main-menu">
          ...
          if($level){
            ...
            <li>
              <a href="input_data_calon.php"><i class="fa fa-user "></i>Input Data Calon</a>
            </li>
      ...
      <div style="width: 800px;margin: 0px auto;">
        ...
        <script>
          ...
          var myChart = new Chart(ctx, {
            type: 'bar',
            data: {
              labels: ["CALON 1", "CALON 2", "CALON 69", "CALON 4"],
              datasets: [{
                label: 'Jumlah Suara',
                data: [
                <?php 
                $calon1 = mysqli_query($koneksi,"select * from tbl_calon where nomor_calon='1'");
                echo mysqli_num_rows($calon1);
                ?>, 
                <?php 
                $calon2 = mysqli_query($koneksi,"select * from tbl_calon where nomor_calon='2'");
                echo mysqli_num_rows($calon2);
                ?>, 
                <?php 
                $calon3 = mysqli_query($koneksi,"select * from tbl_calon where nomor_calon='69'");
                echo mysqli_num_rows($calon3);
                ?>,
                <?php 
                $calon4 = mysqli_query($koneksi,"select * from tbl_calon where nomor_calon='4'");
                echo mysqli_num_rows($calon4);
                ?>
                ],
                backgroundColor: [
                'rgba(255, 99, 132, 0.2)',
